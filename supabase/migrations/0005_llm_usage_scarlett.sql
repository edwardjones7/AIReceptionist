-- 0005 (scarlett) — LLM token-cost tracking for the co-located Elenos DB.
-- RUN THIS ONE on the Elenos project (it runs the `scarlett` schema).
-- Adds scarlett.llm_usage and folds LLM cost into the analytics RPCs.
-- Idempotent.
...
drop function if exists scarlett.tenant_activity_series(text, timestamptz, text);
create function scarlett.tenant_activity_series(
  p_tenant_id text,
  p_start     timestamptz,
  p_bucket    text
) returns table (
...
drop function if exists scarlett.tenant_range_stats(text, timestamptz);
create function scarlett.tenant_range_stats(
  p_tenant_id text,
  p_start     timestamptz
) returns table (
  calls          bigint,
...
drop function if exists scarlett.tenants_range_stats(timestamptz);
create function scarlett.tenants_range_stats(
  p_start timestamptz
) returns table (
  tenant_id      text,
  calls          bigint,
...
grant all privileges on scarlett.llm_usage to service_role;
